...
contract MockVault is AccessControl, IVault {
    ...
    function deposit(IERC20 _token, uint256 _amount) external {
        ...
        balances[address(_token)][msg.sender] += _amount;
        emit Deposit(address(_token),msg.sender, _amount);
    }

    /// ---------- 用户取款 ----------
    function withdraw(IERC20 _token, uint256 _amount) external {
        require(balances[address (_token)][msg.sender] >= _amount, "Insufficient balance");

        balances[address (_token)][msg.sender] -= _amount;
        bool success = _token.transfer(msg.sender, _amount);
        require(success, "Transfer failed");

        emit Withdraw(address (_token), msg.sender, _amount);
    }
    ...
    function claim(IERC20 _token, address _to, uint256 _amount, bool _withdrawal) onlyOperator external {
        ...
        if(_withdrawal) {
            ...
            balances[address(_token)][_to] += _amount;
            emit Claim(address(_token), _to, _amount);
        }
    }
